...
vi.mock("./education.js", () => ({
  ...
  getEducationSkill: (skillId: string) =>
    skillId === "writing-coach"
      ? { id: "writing-coach", name: "写作教练", description: "", ages: "", scenario: "", file: "writing-coach.md", content: "# 写作教练" }
      : null,
}));

const { getEffectiveSkill, getChildProfile, updateChildProfile, clearChildProfile, listChildProfiles } = await import(
  "./personalization.js"
);
...
describe("教育方式按孩子维度分层", () => {
  ...
  it("读取别的家庭的孩子会被拒绝", async () => {
    ...
    await expect(getEffectiveSkill("family-1", "writing-coach", "child-other")).rejects.toThrow(
      "学生不存在或不属于当前家庭",
    );
  });
  ...
  it("写入孩子级配置时记录孩子维度并带上 familyId", async () => {
    ...
    expect(childSkillUpsert).toHaveBeenCalledWith(
    ...
    expect(policyChangeCreate).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("清空孩子级配置后回到继承家庭设置", async () => {
    ...
    expect(policyChangeCreate).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ childId: "child-1", type: "child_profile_clear" }) }),
    );
  });
