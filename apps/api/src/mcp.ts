...
export async function registerMcpHttp(app: FastifyInstance) {
  function getCredentials(request: any) {
  ...
  async function resolveFamily(request: any) {
    ...
    if (legacyToken) return resolveFamilyByMcpToken(legacyToken);
    // Older clients sent the family token as a Bearer token.
    if (bearerToken) return resolveFamilyByMcpToken(bearerToken);
    return null;
  }
  ...
  app.post("/mcp", async (request, reply) => {
    ...
    if (!familyId) return unauthorized(reply);
