 ...
 */
function resolveEducationLayers(family: FamilyLayer, profile: CustomizationLayer | null, childProfile: CustomizationLayer | null) {
  const familySettings = {
    ...
    parentGoals: (profile?.parentGoals ?? []) as string[],
  };
  const effective = {
  ...
  const childOverrides: string[] = (["philosophy", "communicationStyle", "strictness"] as const).filter(
    (key) => Boolean(childProfile?.[key]) && childProfile?.[key] !== familySettings[key],
  );
  if (childProfile?.parentGoals?.length) childOverrides.push("parentGoals");
...
export async function listFamilyPolicies(familyId: string, childId?: string) {
...
export async function getChildProfile(familyId: string, childId: string, skillId: string) {
...
export async function listChildProfiles(familyId: string, childId: string) {
...
export async function updateChildProfile(familyId: string, childId: string, skillId: string, input: {
  philosophy?: string;
...
/** 清空孩子级配置，回到继承家庭设置的状态。 */
export async function clearChildProfile(familyId: string, childId: string, skillId: string, createdBy = "parent") {
  const child = await assertChildInFamily(familyId, childId);
  const before = await prisma.childSkillProfile.findUnique({
    where: { childId_skillId: { childId, skillId } },
  });
 ...
 */
export async function getEffectiveSkill(familyId: string, skillId: string, childId?: string) {
  ...
  const hasFamilyConfig = Boolean(
    profile || family?.educationPhilosophy || family?.communicationStyle || family?.strictness,
  );
