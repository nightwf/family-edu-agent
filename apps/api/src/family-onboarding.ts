...
export async function findOrCreateWechatUser(openid: string, unionid?: string) {
...
export async function ensureFamilyJoinCode(familyId: string) {
  const family = await prisma.family.findUnique({ where: { id: familyId } });
  if (!family) throw new Error("家庭不存在");
  if (family.joinCode) return family.joinCode;
...
export async function createFamilyForUser(userId: string, familyName?: string) {
  const joinCode = await createUniqueFamilyJoinCode();
...
export async function applyToFamilyByJoinCode(userId: string, rawCode: unknown) {
  const joinCode = String(rawCode || "").trim();
  if (!/^\d{6}$/.test(joinCode)) throw new Error("请输入正确的 6 位家庭编码");
  const family = await prisma.family.findUnique({ where: { joinCode }, select: { id: true, name: true } });
  if (!family) throw new Error("家庭编码不存在，请向家庭创建者确认");
  const existingMember = await getActiveFamilyMember(family.id, userId);
  if (existingMember) return { already_member: true, family, request: null };
...
export async function listFamilyJoinRequests(familyId: string) {
...
export async function reviewFamilyJoinRequest(familyId: string, requestId: string, reviewerUserId: string, action: "approved" | "rejected") {
  const joinRequest = await prisma.familyJoinRequest.findFirst({
    where: { id: requestId, familyId, status: "pending" },
  });
  if (!joinRequest) throw new Error("加入申请不存在或已处理");
  if (action === "rejected") {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (!applicant.familyId) await tx.user.update({ where: { id: applicant.id }, data: { familyId } });
