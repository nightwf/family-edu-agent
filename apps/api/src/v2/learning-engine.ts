...
export const SIGNAL_BASE_PRIORITY: Record<SignalType, number> = {
...
function attemptPassed(attempt: { isCorrect: boolean | null; score: number | null }) {
 ...
 */
export function aggregateKnowledgeScore(
  ...
  const delayedReviewPassed = Boolean(
    firstIndependentCorrect
    && ordered.some((item) => (
  ...
  if (totalAttempts > 0) status = "LEARNING";
  if (totalAttempts >= 3 && score >= LOW_MASTERY_THRESHOLD) status = "PARTIAL";
  if (mastered) status = "MASTERED";
  if (previousStatus === "MASTERED" && !mastered && totalAttempts > 0) status = "NEEDS_REVIEW";
...
async function requireQuestionType(familyId: string, questionTypeId: string) {
  const questionType = await prisma.questionType.findFirst({ where: { id: questionTypeId, familyId } });
  if (!questionType) throw new LearningEngineError("题型不存在或不属于当前家庭", 404);
  return questionType;
}

async function requireKnowledgeNodes(familyId: string, nodeIds: string[]) {
  const nodes = await prisma.knowledgeNode.findMany({ where: { familyId, id: { in: nodeIds } } });
  if (nodes.length !== nodeIds.length) throw new LearningEngineError("知识节点不存在或不属于当前家庭", 404);
  return nodes;
}

function linkValues(links: Array<{ knowledge_node_id: string; role?: string; weight?: number }>) {
  ...
  if (!cleaned.length) throw new LearningEngineError("至少需要一个有效的 knowledge_node_id");
  return cleaned;
}

/** 题型关联知识节点，重复调用按节点覆盖，不产生重复关联。 */
export async function linkQuestionTypeKnowledgeNodes(
  familyId: string,
  questionTypeId: string,
  links: Array<{ knowledge_node_id: string; role?: string; weight?: number }>,
) {
...
export async function listQuestionTypeKnowledgeNodes(familyId: string, questionTypeId: string) {
...
export async function unlinkQuestionTypeKnowledgeNode(familyId: string, questionTypeId: string, knowledgeNodeId: string) {
  await requireQuestionType(familyId, questionTypeId);
...
export async function linkQuestionKnowledgeNodes(
  familyId: string,
  questionId: string,
  links: Array<{ knowledge_node_id: string; role?: string; weight?: number }>,
) {
  const question = await prisma.question.findFirst({ where: { id: questionId, familyId } });
  if (!question) throw new LearningEngineError("题目不存在或不属于当前家庭", 404);
  const cleaned = linkValues(links);
  await requireKnowledgeNodes(familyId, cleaned.map((item) => item.knowledgeNodeId));
  for (const link of cleaned) {
...
export async function listQuestionKnowledgeNodes(familyId: string, questionId: string) {
...
export async function resolveKnowledgeNodeIds(familyId: string, questionId: string, questionTypeId: string) {
  const questionLinks = await prisma.questionKnowledgeNode.findMany({ where: { familyId, questionId } });
  if (questionLinks.length) return questionLinks.map((item) => item.knowledgeNodeId);
 ...
 */
export async function refreshKnowledgeStateFromAttempt(
  familyId: string,
  childId: string,
  questionId: string,
  questionTypeId: string,
  ...
  if (!nodeIds.length) return { updated: [] as Array<{ knowledge_node_id: string; status: string; score: number }> };
  ...
  const updated: Array<{ knowledge_node_id: string; status: string; score: number }> = [];

  for (const knowledgeNodeId of nodeIds) {
    const relatedTypeIds = new Set(
      typeLinks.filter((item) => item.knowledgeNodeId === knowledgeNodeId).map((item) => item.questionTypeId),
    );
    ...
    const previousCalculated = existing
      ? existing.manualStatus
        ? ((existing.evidence as Record<string, unknown> | null)?.calculated_status as string | undefined) ?? null
        : existing.status
      : null;
    const result = aggregateKnowledgeScore(
      attempts.map((item) => ({
 ...
 * 不做情绪或心理推断。
 */
export async function detectLearningSignals(familyId: string, childId: string, now = new Date()): Promise<SignalDraft[]> {
 ...
 * 这样首页只显示当前真正需要注意的问题。
 */
export async function syncLearningSignals(familyId: string, childId: string, now = new Date()) {
  ...
  for (const draft of drafts) {
    await prisma.learningSignal.upsert({
      ...
      update: {
        ...
        reason: draft.reason,
        evidence: draft.evidence as any,
  ...
  for (const signal of openSignals) {
    if (activeKeys.has(signal.dedupeKey)) continue;
 ...
 */
export async function resolveLearningSignal(
  familyId: string,
  signalId: string,
  ...
  if (!signal) throw new LearningEngineError("信号不存在或不属于当前家庭", 404);
  const evidence = {
...
export function rankLearningPriorities(
  signals: RankingInput[],
  options: { goalText?: string; now?: Date } = {},
) {
 ...
 */
export async function getLearningPriorities(
  familyId: string,
  childId: string,
  ...
  if (!child) throw new LearningEngineError("学生不存在或不属于当前家庭", 404);
...
export async function ensurePlanningRequest(familyId: string, childId: string, now = new Date()) {
  ...
  return prisma.planningRequest.create({
    data: {
      ...
      signalIds: priorities.priorities.map((item) => item.signal_id) as any,
      prioritySnapshot: priorities.priorities as any,
    },
  });
}
...
export async function listPlanningRequests(
  familyId: string,
  input: { child_id?: string; status?: string; limit?: number } = {},
) {
...
export async function getPlanningRequest(familyId: string, planningRequestId: string) {
  const request = await prisma.planningRequest.findFirst({ where: { id: planningRequestId, familyId } });
  if (!request) throw new LearningEngineError("待规划事项不存在或不属于当前家庭", 404);
  return request;
}

export async function updatePlanningRequestStatus(
  familyId: string,
...
export async function listRecommendationOutcomes(
  familyId: string,
  input: { child_id?: string; source_type?: string; source_id?: string; limit?: number } = {},
) {
