...
import { getChatProvider, hasChatCredentials, pickModel, type ChatProvider } from "../tutor/llm/index.js";
...
const itemTypes = ["SCHOOL_HOMEWORK", "CHILD_TASK", "PARENT_ACTION", "AGENT_TASK", "RETEST"] as const;
...
export type AiPlanOutput = z.infer<typeof plannerOutputSchema>;
...
export function parseAiPlanOutput(text: string): AiPlanOutput {
  let value: unknown;
  try {
...
async function collectPlan(provider: ChatProvider, context: unknown) {
...
export async function generateAiPlanDraft(
  familyId: string,
  planningRequestId: string,
  actor: { type: string; id?: string },
  provider: ChatProvider = getChatProvider(),
) {
  ...
  if (provider.name !== "fake" && !hasChatCredentials()) throw new Error("系统 AI 模型尚未配置");
  if (provider.name !== "fake" && !pickModel(false)) throw new Error("系统 AI 对话模型尚未配置");
  ...
  if (!locked.count) throw new Error("计划正在生成，请勿重复提交");
  ...
  try {
    ...
    const goals = await proposeStageGoals(
      familyId,
      request.childId,
      output.goals.map((goal) => ({ ...goal, startDate, endDate })),
      actor,
    );
...
export async function confirmAiPlanDraft(
  familyId: string,
  planningRequestId: string,
  actor: { type: string; id?: string },
) {
  ...
  const draft = (request.aiDraft || {}) as any;
  ...
  const otherGoalIds = Array.isArray(draft.candidate_goal_ids)
    ? draft.candidate_goal_ids.filter((id: unknown) => typeof id === "string" && id !== request.stageGoalId)
    : [];
