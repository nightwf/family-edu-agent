...
const { buildAdvice, buildSubjectRow, classifySubject, describeSubjectChange, getSubjectDetail, getSubjectOverview, rankSubjectRows } =
  await import("./subject-overview.js");
...
function mastery(overrides: Record<string, unknown> = {}) {
  return {
    name: "看图列式",
    score: 54,
    status: "learning",
...
describe("学科排序", () => {
  it("需重点排最前，稳定与材料不足排后面", () => {
    const rows = [
      { status: "thin" as const, mastery_score: null },
      { status: "steady" as const, mastery_score: 80 },
      { status: "focus" as const, mastery_score: 54 },
      { status: "progress" as const, mastery_score: 76 },
    ];
  ...
  it("同为需重点时掌握度低的排前面", () => {
    const rows = [
      { status: "focus" as const, mastery_score: 70 },
      { status: "focus" as const, mastery_score: 42 },
    ];
...
describe("规划建议由规则生成", () => {
  ...
  it("重复出错给出变式加迁移题，并带上题型通过标准", () => {
    const advice = buildAdvice(
      { type: "REPEATED_ERROR", label: "看图列式", reason: "重复出错 2 次", priority_score: 115 },
      undefined,
      { solutionMethod: "先圈出总量再列式", masteryCriteria: criteria },
      { subject: "数学" },
    );
  ...
  it("复测到期给出短时复测安排", () => {
    const advice = buildAdvice(
      { type: "REVIEW_DUE", label: "乘法口诀", reason: "已到复测时间", priority_score: 90 },
      undefined,
