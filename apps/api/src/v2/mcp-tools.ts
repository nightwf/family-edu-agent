...
async function safe(action: () => Promise<unknown>) {
  try {
    ...
    return {
      isError: true,
      content: [{ type: "text" as const, text: error instanceof Error ? error.message : "操作失败" }],
    };
  }
}

export function registerV2McpTools(server: McpServer, familyId: string) {
  server.tool("get_family_policy", "读取当前家庭的边界、目标和原则。", {}, async () =>
    safe(() => getFamilyPolicy(familyId)),
  );
  ...
  server.tool(
    ...
    async (input) =>
      safe(() =>
        proposeFamilyPolicyChange(familyId, {
          type: input.type,
  ...
  server.tool(
    ...
    async (input) =>
      safe(() =>
        reviewFamilyPolicyChange(familyId, input.change_id, input.action, { type: "parent" }),
      ),
  );

  server.tool("get_child_state", { child_id: z.string() }, async ({ child_id }) =>
    safe(() => getChildState(familyId, child_id)),
  );
  ...
  server.tool(
    ...
    async (input) =>
      safe(() =>
        reviewEvidenceRecord(familyId, input.evidence_id, input.action, { type: "parent" }, input.note),
      ),
  );

  server.tool("get_planning_context", { child_id: z.string(), purpose: z.string().optional() }, async ({ child_id }) =>
    safe(() => getPlanningContext(familyId, child_id)),
  );
  ...
  server.tool("list_stage_goals", { child_id: z.string().optional(), status: z.string().optional() }, async (input) =>
    safe(() => listStageGoals(familyId, { childId: input.child_id, status: input.status })),
  );

  server.tool("get_stage_goal", { goal_id: z.string() }, async ({ goal_id }) =>
    safe(() => getStageGoal(familyId, goal_id)),
  );
  ...
  server.tool(
    ...
    async (input) =>
      safe(() =>
        confirmStageGoal(familyId, input.goal_id, input.action, { type: "parent" }, input.changes),
      ),
  );
  ...
  server.tool("get_weekly_plan", { plan_id: z.string() }, async ({ plan_id }) =>
    safe(() => getWeeklyPlan(familyId, plan_id)),
  );
  ...
  server.tool(
    ...
    async (input) =>
      safe(() =>
        updatePlanItemStatus(familyId, input.plan_item_id, {
          status: input.status,
  ...
  server.tool("list_source_documents", { subject: z.string().optional(), grade: z.string().optional() }, async (input) =>
    safe(() => listSourceDocuments(familyId, input)),
  );
  ...
  server.tool("list_knowledge_nodes", { subject: z.string().optional(), grade: z.string().optional(), source_document_id: z.string().optional() }, async (input) =>
    safe(() => listKnowledgeNodes(familyId, input)),
  );
  ...
  server.tool(
    ...
    async (input) =>
      safe(() => saveKnowledgeNodesBatch(familyId, input.source_document_id, input.nodes)),
  );

  server.tool(
    "save_knowledge_relations_batch",
    "保存同一来源文档中知识点之间的关系，推荐使用 PREREQUISITE_OF 表达前置依赖，并补充 hard/soft 强度与原因。",
    {
      source_document_id: z.string(),
      relations: z.array(
        z.object({
  ...
  server.tool("get_knowledge_context", { child_id: z.string(), knowledge_node_id: z.string() }, async (input) =>
    safe(() => getKnowledgeContext(familyId, input.child_id, input.knowledge_node_id)),
  );

  server.tool(
    "update_child_knowledge_state",
    {
      child_id: z.string(),
    ...
    async (input) =>
      safe(() =>
        upsertChildKnowledgeState(familyId, {
          childId: input.child_id,
  ...
  server.tool("list_education_methods", { category: z.string().optional() }, async (input) =>
    safe(async () => {
  ...
  server.tool(
    "get_child_relationship",
    { child_id: z.string() },
    async ({ child_id }) => safe(() => getLatestRelationship(familyId, child_id)),
  );
  ...
  server.tool(
    "list_child_relationship_history",
    {
      child_id: z.string(),
      limit: z.number().int().min(1).max(100).optional(),
      offset: z.number().int().min(0).optional(),
    },
    async ({ child_id, limit, offset }) =>
      safe(() => listRelationshipHistory(familyId, child_id, limit, offset)),
  );
  ...
  server.tool(
    ...
    "读取学生各学科的整体概览（含没有数据的学科）。用于回答“哪一科要优先处理”“各科现在什么情况”。",
    { child_id: z.string() },
    async ({ child_id }) => safe(() => getSubjectOverview(familyId, child_id)),
  );
