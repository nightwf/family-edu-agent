...
describe("goal and plan", () => {
  ...
  it("rejects goals shorter than four weeks", async () => {
    await expect(
      proposeStageGoals("family-1", "child-1", [
  ...
  it("creates a weekly plan only for a confirmed or active goal", async () => {
    ...
    expect(weeklyPlanCreate).toHaveBeenCalledWith(
  ...
  it("accepts Chinese plan item types and normalizes them to enum keys", async () => {
    ...
    expect(payload.data.items.create.map((item: any) => item.type)).toEqual(["SCHOOL_HOMEWORK", "RETEST"]);
  });

  it("lists all valid plan item types when the type is unknown", async () => {
    await expect(
      createWeeklyPlan("family-1", "goal-1", "2026-09-07", [{ type: "布鲁姆记忆层", title: "每日 10 题" }]),
    ).rejects.toThrow(/SCHOOL_HOMEWORK（学校作业）/);
  ...
  it("requires completion evidence when completing a plan item", async () => {
    ...
    await expect(
      updatePlanItemStatus("family-1", "item-1", { status: "COMPLETED", evidence: null }),
    ).rejects.toThrow("完成任务必须提供完成证据");
  });
});
