...
/** 取最近 N 轮对话，按时间正序返回。 */
export async function buildHistory(conversationId: string, windowTurns = env.TUTOR_CONTEXT_WINDOW_TURNS): Promise<ChatMessage[]> {
...
export function needsSummarize(messageCount: number) {
...
export async function extractEvidence(options: {
  ...
  if (!draft.observedBehavior?.trim()) return null;
  ...
  if (existing && existing.observedBehavior === draft.observedBehavior) return null;
...
export function draftFromTurn(userMessage: string, assistantText: string): EvidenceDraft | null {
