...
function jsonResponse(body: any, headers: Record<string, string> = {}, status = 200) {
  return {
...
describe("语音识别（ASR）", () => {
  ...
  it("正文为空对象时，从响应头取出 code 与 message", async () => {
    stubFetch(() =>
      jsonResponse({}, { "x-api-status-code": "45000001", "x-api-message": "request invalid" }),
    );
    await expect(createVolcAsr(asrConfig).transcribe(Buffer.from("audio"), "mp3")).rejects.toThrow(
      /code=45000001.*request invalid/s,
    );
  });
