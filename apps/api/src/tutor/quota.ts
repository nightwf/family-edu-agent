...
export async function getQuotaState(familyId: string, childId?: string | null): Promise<QuotaState> {
  const since = startOfToday();
  const where: any = { familyId, role: "user", createdAt: { gte: since } };
  ...
  const usedTokens = tokenRows.reduce(
