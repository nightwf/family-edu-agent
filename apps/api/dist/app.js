 ...
 */
async function createWechatSessionResponse(app, user, familyId) {
...
async function requireUserAuth(request, reply) {
    try {
        await request.jwtVerify();
        const payload = request.user;
        if (!payload?.sub)
...
export async function buildApp() {
    ...
    app.post("/api/auth/wechat/attach", async (request, reply) => {
        try {
            ...
            if (!code || !email || !password)
                return reply.code(400).send({ error: "请填写完整账号信息" });
            const wechat = await exchangeWechatCode(String(code));
            ...
            if (!user.familyId)
                return reply.code(400).send({ error: "该账号还没有家庭，请直接使用微信登录" });
    ...
    app.get("/api/onboarding/state", { preHandler: requireUserAuth }, async (request, reply) => {
    ...
    app.post("/api/onboarding/family", { preHandler: requireUserAuth }, async (request, reply) => {
        const auth = getUserAuth(request);
        if (!auth)
            return reply.code(401).send({ error: "未登录或登录已过期" });
        const existingCount = await prisma.familyMember.count({ where: { userId: auth.id, status: "active" } });
        if (existingCount > 0)
            return reply.code(409).send({ error: "该账号已经有家庭，不能重复创建" });
        const { family, user } = await createFamilyForUser(auth.id, request.body?.name);
        return reply.code(201).send(await createWechatSessionResponse(app, user, family.id));
    });
    app.post("/api/onboarding/family/join-request", { preHandler: requireUserAuth }, async (request, reply) => {
        const auth = getUserAuth(request);
        if (!auth)
            return reply.code(401).send({ error: "未登录或登录已过期" });
        try {
            const result = await applyToFamilyByJoinCode(auth.id, request.body?.join_code);
            if (result.already_member)
                return reply.code(409).send({ error: "你已经是该家庭的成员" });
    ...
    app.get("/api/onboarding/family/join-requests", { preHandler: requireAuth }, async (request) => {
        return listFamilyJoinRequests(getAuth(request).familyId);
    });
    app.post("/api/onboarding/family/join-requests/:requestId/review", { preHandler: requireAuth }, async (request, reply) => {
        const auth = getAuth(request);
        const { requestId } = request.params;
        const action = request.body?.action;
        if (!["approved", "rejected"].includes(action))
            return reply.code(400).send({ error: "action 只能是 approved 或 rejected" });
        const isOwner = await requireOwner(auth.familyId, auth.id);
        if (!isOwner)
            return reply.code(403).send({ error: "只有家庭创建者可以审核加入申请" });
        try {
