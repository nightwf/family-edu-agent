...
import { listFamilyPolicies, getEffectiveSkill, updateFamilyProfile, listChildProfiles, updateChildProfile, clearChildProfile, proposePolicyChange, reviewPolicyChange, getPolicyHistory, createSkillOverride, listSkillOverrides, } from "./personalization.js";
...
export function createEducationMcpServer(familyId = env.MCP_FAMILY_ID) {
    ...
    server.tool("get_sync_spec", "读取禾芽最新版详细同步规范。新会话先调用 get_agent_bootstrap；工具变化、复杂任务或不确定应保存什么时再调用本工具。", {}, async () => textResult({
        ...
        child_dimension_rule: "同一家庭的不同孩子可以有各自的教育方式。执行某个孩子的任务前先调用 get_effective_skill 并传 child_id，拿到「家庭策略 + 孩子个体调整」合并后的结果；只有传了 child_id 才会应用孩子级配置。家庭级配置用 list_family_policies / update_family_policy（不传 child_id），孩子级配置用 get_child_education_profile / update_child_education_profile。不要把一个孩子的偏好套用到另一个孩子身上。",
    ...
    server.tool("list_family_policies", "读取家庭级教育配置。传入 child_id 时会额外返回该孩子的个体配置和两者合并后的最终设置。", { family_id: z.string().optional(), child_id: z.string().optional() }, async ({ child_id }) => {
    ...
    server.tool("get_child_education_profile", "读取某个孩子各教育 Skill 的个体配置，以及与家庭设置合并后的最终生效设置。", { family_id: z.string().optional(), child_id: z.string() }, async ({ child_id }) => textResult(await listChildProfiles(familyId, child_id)));
    server.tool("update_child_education_profile", "写入某个孩子的个体教育方式（教育理念、沟通风格、严格程度、家长目标、学习特点）。只覆盖传入的字段，其余字段继续继承家庭设置。", {
        family_id: z.string().optional(),
        child_id: z.string(),
        skill_id: z.string(),
        philosophy: z.string().optional(),
        ...
        return textResult(await updateChildProfile(familyId, input.child_id, input.skill_id, {
            philosophy: input.philosophy,
            communicationStyle: input.communication_style,
            strictness: input.strictness,
            parentGoals: input.parent_goals,
    ...
    server.tool("get_wrong_question", "读取错题原题、解析、错误原因、掌握证据、练习记录、关联试卷和教学任务。", { wrong_question_id: z.string() }, async ({ wrong_question_id }) => (questionBankResult(() => getWrongQuestion(familyId, wrong_question_id))));
    ...
    server.tool("delete_wrong_question", "删除无关联证据的错题；有关联练习、试卷或任务时按数据完整性规则归档。", { wrong_question_id: z.string() }, async ({ wrong_question_id }) => (questionBankResult(() => deleteWrongQuestion(familyId, wrong_question_id))));
    ...
    server.tool("recalculate_wrong_question_mastery", "根据全部关联作答重新计算错题掌握度，同时重算对应题型掌握度。", { wrong_question_id: z.string() }, async ({ wrong_question_id }) => (questionBankResult(() => recalculateWrongQuestionMastery(familyId, wrong_question_id))));
    ...
    server.tool("get_practice_paper", "读取试卷题目、答案解析、关联错题和学生作答。", { practice_paper_id: z.string() }, async ({ practice_paper_id }) => (questionBankResult(() => getPracticePaper(familyId, practice_paper_id))));
    ...
    server.tool("delete_practice_paper", "删除未作答试卷；已有作答记录时归档。不会删除题库题目。", { practice_paper_id: z.string() }, async ({ practice_paper_id }) => (questionBankResult(() => deletePracticePaper(familyId, practice_paper_id))));
    ...
    server.tool("get_remediation_plan", "读取教学规划诊断、目标、策略、任务和完成证据。", { remediation_plan_id: z.string() }, async ({ remediation_plan_id }) => (questionBankResult(() => getRemediationPlan(familyId, remediation_plan_id))));
