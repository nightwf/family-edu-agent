...
function toneForStatus(status) {
    if (["mastered", "CONFIRMED", "CORRECTED"].includes(status))
        return "positive";
    if (["needs_review", "pending_correction"].includes(status))
        return "attention";
    return "neutral";
}
function snapshotSummary(snapshot) {
    ...
    return compact(summary.headline || summary.judgment || summary.summary || indicators.headline || indicators.judgment, "系统已根据近期证据更新孩子状态");
}
export function buildGrowthTimeline(input) {
