...
function randomOpaque(prefix, bytes = 32) {
...
export function isAllowedWorkbuddyRedirectUri(value) {
    try {
...
async function findValidBinding(publicId) {
...
async function findValidWebLogin(publicId) {
...
export async function revokeOAuthConnection(familyId, connectionId) {
...
export async function registerOAuthRoutes(app, options) {
    ...
    app.post("/oauth/register", async (request, reply) => {
        ...
        if (!redirectUris.length || redirectUris.some((uri) => !isAllowedWorkbuddyRedirectUri(uri))) {
    ...
    app.get("/oauth/authorize", async (request, reply) => {
        ...
        if (!client || client.status !== "active")
            return oauthError(reply, 400, "invalid_client", "客户端未注册或已停用");
        const redirectUri = String(query.redirect_uri || "");
        if (!client.redirectUris.includes(redirectUri))
            return oauthError(reply, 400, "invalid_request", "redirect_uri 与注册信息不一致");
        if (query.response_type !== "code")
            return oauthError(reply, 400, "unsupported_response_type", "仅支持 authorization code");
    ...
    app.post("/oauth/token", async (request, reply) => {
        ...
        if (!client || client.status !== "active")
            return oauthError(reply, 401, "invalid_client", "客户端无效");
        ...
        if (grantType === "authorization_code") {
            ...
            if (claimed.count !== 1)
                return oauthError(reply, 400, "invalid_grant", "授权码已经使用");
    ...
    app.get("/api/oauth/bindings/:publicId/qrcode", async (request, reply) => {
        ...
        const { browser_secret: browserSecret } = request.query;
        ...
        if (binding.status !== "pending")
            return reply.code(410).send({ error: "授权会话已结束" });
        try {
    ...
    app.get("/api/oauth/bindings/:publicId/status", async (request, reply) => {
        ...
        if (binding.status === "approved" && binding.authorizationCodeCipher) {
            ...
            if (binding.state)
                callback.searchParams.set("state", binding.state);
    ...
    app.post("/api/oauth/bindings/:publicId/wechat-session", async (request, reply) => {
        ...
        if (!binding || binding.status !== "pending")
            return reply.code(410).send({ error: "扫码授权已过期，请返回 WorkBuddy 重试" });
        try {
    ...
    app.post("/api/oauth/bindings/:publicId/approve", { preHandler: options.requireUserAuth }, async (request, reply) => {
        const { publicId } = request.params;
        const auth = options.getUserAuth(request);
        if (!auth)
            return reply.code(401).send({ error: "未登录或登录已过期" });
        const familyId = String(request.body?.familyId || "").trim();
        const binding = await findValidBinding(String(publicId));
        if (!binding || binding.status !== "pending")
            return reply.code(410).send({ error: "扫码授权已过期，请返回 WorkBuddy 重试" });
        const member = await getActiveFamilyMember(familyId, auth.id);
        if (!member)
            return reply.code(403).send({ error: "你不是所选家庭的管理者" });
    ...
    app.get("/api/auth/wechat/web/:publicId/qrcode", async (request, reply) => {
        ...
        if (session.status !== "pending")
            return reply.code(410).send({ error: "登录会话已结束" });
        try {
    ...
    app.post("/api/auth/wechat/web/:publicId/wechat-session", async (request, reply) => {
        ...
        if (!session || session.status !== "pending")
            return reply.code(410).send({ error: "扫码登录已过期，请刷新网页重试" });
        try {
            const { code } = request.body;
            if (!code)
                return reply.code(400).send({ error: "缺少微信登录 code" });
    ...
    app.post("/api/auth/wechat/web/:publicId/approve", { preHandler: options.requireUserAuth }, async (request, reply) => {
        const { publicId } = request.params;
        const auth = options.getUserAuth(request);
        if (!auth)
            return reply.code(401).send({ error: "未登录或登录已过期" });
        ...
        if (!session || session.status !== "pending")
            return reply.code(410).send({ error: "扫码登录已过期，请刷新网页重试" });
        const familyId = String(request.body?.familyId || auth.familyId || "").trim();
        const member = familyId ? await getActiveFamilyMember(familyId, auth.id) : null;
        if (!member)
            return reply.code(403).send({ error: "请先在微信里选择或创建一个家庭" });
    ...
    app.post("/api/auth/wechat/web/exchange", async (request, reply) => {
        ...
        if (claimed.count !== 1)
            return reply.code(400).send({ error: "登录授权已经使用" });
        const user = await prisma.user.findUnique({ where: { id: session.userId } });
        if (!user || user.status !== "active")
            return reply.code(403).send({ error: "账号不可用" });
