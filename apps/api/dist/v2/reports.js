...
export async function createWeeklyReviewDraft(familyId, childId, weekStart) {
...
export async function confirmWeeklyReview(familyId, childId, weekStart, adjustments, actor = { type: "parent" }) {
    await assertChildInFamily(familyId, childId);
    ...
    const updated = await prisma.weeklyReview.update({
        ...
        data: {
            status: "confirmed",
            parentAdjustments: (adjustments ?? undefined),
...
export async function createStageReport(familyId, childId, stageGoalId) {
    ...
    const verdict = passedAssessments.length > 0 && failedAssessments.length === 0
        ? "improved"
