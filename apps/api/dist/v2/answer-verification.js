...
export function evaluateArithmetic(input) {
    ...
    if (value === undefined || stack.length || !Number.isFinite(value))
        return null;
    return value;
}
...
export function verifyQuestionAnswer(question) {
    ...
    if (format === "short_answer" || format === "essay") {
        const hasRubric = Boolean(question.scoringRubric) && Object.keys(question.scoringRubric || {}).length > 0;
        return {
    ...
    if (format === "single_choice" || format === "multiple_choice" || format === "true_false") {
        ...
        const answers = asArray(answer).map((item) => String(isPlainObject(item) ? item.key ?? item : item).trim());
        const errors = [];
        if (format === "multiple_choice" && answers.length < 2)
            errors.push("多选题标准答案应包含两个及以上选项");
        if (format !== "multiple_choice" && answers.length !== 1)
            errors.push("单选题标准答案应只有一个选项");
        if (format === "true_false") {
            const flag = normalizeText(answers[0]);
            if (!["true", "false", "对", "错", "正确", "错误", "t", "f"].includes(flag))
                errors.push("判断题答案应为对或错");
        }
        else if (keys.length) {
            const normalizedKeys = keys.map((item) => normalizeText(item));
            for (const item of answers) {
                if (!normalizedKeys.includes(normalizeText(item)))
                    errors.push(`标准答案「${item}」不在选项中`);
            }
        }
        else {
...
export function verifyQuestionAnswers(questions) {
