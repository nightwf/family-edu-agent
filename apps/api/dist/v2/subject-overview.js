 ...
 */
export const SUBJECT_STATUS = ["focus", "progress", "steady", "thin"];
export const SUBJECT_STATUS_TEXT = {
    focus: "需重点",
    progress: "进步中",
    steady: "状态稳定",
    thin: "材料不足",
};
const STATUS_ORDER = { focus: 0, progress: 1, steady: 2, thin: 3 };
const FOCUS_SIGNALS = ["PREREQUISITE_GAP", "REPEATED_ERROR", "LOW_MASTERY"];
...
function subjectMasteryScore(masteries) {
    if (!masteries.length)
        return null;
...
function reviewDueNames(masteries, wrongItems, now) {
 ...
 */
export function classifySubject(input, now = new Date()) {
    ...
    if (!hasAnyData) {
        return { status: "thin", score: null, dueNames, blockingSignals, reason: "这一科还没有作答和练习记录" };
    }
    if (blockingSignals.length || (score !== null && score < LOW_MASTERY_SCORE) || dueNames.length) {
        ...
        return { status: "focus", score, dueNames, blockingSignals, reason };
    }
    ...
    if (input.attempts7d > 0 && score !== null && score >= 70 && !needsReview) {
        return { status: "progress", score, dueNames, blockingSignals, reason: "最近一周保持练习，且没有待处理的薄弱点" };
    }
    return { status: "steady", score, dueNames, blockingSignals, reason: "当前没有需要额外处理的薄弱点" };
}
function weakItems(input) {
    const rows = [
        ...input.masteries
            .filter((item) => item.score < WEAK_SCORE || item.status === "needs_review")
            .map((item) => ({ name: item.name, score: item.score })),
        ...input.wrongItems.map((item) => ({ name: item.name, score: null })),
    ];
    return rows.slice(0, 5);
}
/** 一句话变化说明，优先讲最需要家长知道的事。 */
export function describeSubjectChange(input, now = new Date()) {
...
export function buildSubjectRow(input, now = new Date()) {
    const judgement = classifySubject(input, now);
    return {
...
export function rankSubjectRows(rows) {
...
async function collectSubjectInputs(familyId, childId, now) {
    const child = await prisma.child.findFirst({ where: { id: childId, familyId } });
    if (!child)
        throw new SubjectOverviewError("学生不存在或不属于当前家庭", 404);
    ...
    for (const item of attempts) {
        ...
        if (item.attemptedAt >= sevenDaysAgo)
            target.attempts7d += 1;
    }
...
function buildOverall(rows, priorities) {
    const focusRows = rows.filter((row) => row.status === "focus");
    const scored = rows.map((row) => row.mastery_score).filter((score) => typeof score === "number");
    const metrics = {
...
export async function getSubjectOverview(familyId, childId, now = new Date()) {
...
export async function getSubjectDetail(familyId, childId, subject, now = new Date()) {
    const { buckets, priorities } = await collectSubjectInputs(familyId, childId, now);
    const key = normalizeSubject(subject);
    const input = buckets.get(key);
    if (!input)
        throw new SubjectOverviewError("没有这一科的学习记录", 404);
    ...
    const signalOrder = ["PREREQUISITE_GAP", "REPEATED_ERROR", "REVIEW_DUE", "LOW_MASTERY", "VARIATION_GAP"];
...
function buildJudgement(row, subject) {
    if (row.status === "thin")
        return `${subject}还没有足够的记录，暂时无法判断，先补一次练习。`;
    if (row.status === "focus") {
        return `${subject}整体掌握度 ${row.mastery_score ?? "—"} 分，${row.top_reason}，本周需要重点安排。`;
    }
    if (row.status === "progress")
        return `${subject}整体掌握度 ${row.mastery_score ?? "—"} 分，最近保持练习，按当前节奏继续即可。`;
 ...
 */
export function buildAdvice(signal, fallbackMastery, masteryRow, row) {
    const criteria = normalizeMasteryCriteria(masteryRow?.masteryCriteria);
    const steps = Array.isArray(masteryRow?.standardSteps) ? (masteryRow?.standardSteps).map(String) : [];
