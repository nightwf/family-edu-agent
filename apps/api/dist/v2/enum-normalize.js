...
export function enumHint(spec) {
    return Object.keys(spec).map((key) => `${key}（${spec[key].label}）`).join("、");
}
/**
 * 归一化枚举取值。无法识别时返回 null，由调用方决定是报错还是忽略。
 */
export function normalizeEnumValue(spec, raw) {
    const target = comparable(raw);
    if (!target)
        return null;
    for (const key of Object.keys(spec)) {
        if (comparable(key) === target)
            return key;
        const aliases = spec[key].aliases || [];
        if (aliases.some((alias) => comparable(alias) === target))
            return key;
    }
    return null;
}
 ...
 */
export function requireEnumValue(spec, raw, fieldLabel) {
    const resolved = normalizeEnumValue(spec, raw);
    if (resolved)
        return resolved;
    throw new EnumValueError(`${fieldLabel}取值无效：${raw === undefined || raw === null || raw === "" ? "（空）" : `“${raw}”`}。合法值为 ${enumHint(spec)}，也可以直接写对应的中文名称。`);
}
/** 可选枚举：空值返回 undefined，取值非法则报错。 */
export function optionalEnumValue(spec, raw, fieldLabel) {
...
export const CHILD_KNOWLEDGE_STATUSES = {
