...
export const TASK_STATUSES = ["pending", "in_progress", "completed", "skipped"];
...
export function calculateWrongQuestionMastery(attemptsInput, originalQuestionId, criteriaInput, previousCalculatedStatus = "pending_correction", now = new Date()) {
    ...
    const originalCorrectionPassed = attempts.some((attempt) => (attempt.questionId === originalQuestionId && attempt.isOriginalCorrection && attemptPassed(attempt)));
    ...
    const transferPassed = correctIndependent.some((attempt) => (attempt.variationType === "transfer" || attempt.question.variationType === "transfer" || attempt.question.difficulty === "transfer"));
    const firstIndependentCorrect = correctIndependent[0];
    const delayedMs = criteria.delayedHours * 60 * 60 * 1000;
    const delayedReviewPassed = Boolean(firstIndependentCorrect && correctIndependent.some((attempt) => (attempt.id !== firstIndependentCorrect.id
    ...
    if (latest && !attemptPassed(latest) && (previousCalculatedStatus === "mastered" || requirementsPassed))
...
export async function requireWrongQuestion(familyId, wrongQuestionId) {
    const item = await prisma.wrongQuestionEntry.findFirst({ where: { id: wrongQuestionId, familyId } });
    if (!item)
        throw new QuestionBankError("错题不存在或不属于当前家庭", 404);
    return item;
}
export async function requirePracticePaper(familyId, practicePaperId) {
    const item = await prisma.practicePaper.findFirst({ where: { id: practicePaperId, familyId } });
    if (!item)
        throw new QuestionBankError("练习试卷不存在或不属于当前家庭", 404);
    return item;
}
export async function requireRemediationPlan(familyId, remediationPlanId) {
    const item = await prisma.remediationPlan.findFirst({ where: { id: remediationPlanId, familyId } });
    if (!item)
        throw new QuestionBankError("教学规划不存在或不属于当前家庭", 404);
    return item;
}
export async function saveWrongQuestion(familyId, input) {
    ...
    if (sourceAttemptId) {
        ...
        if (!sourceAttempt)
            throw new QuestionBankError("来源作答不存在、学生不一致或不属于当前家庭", 404);
    }
    ...
    const item = await prisma.$transaction(async (tx) => {
        ...
        if (sourceAttemptId)
            await tx.questionAttempt.update({ where: { id: sourceAttemptId }, data: { wrongQuestionId: saved.id } });
        return saved;
    });
    return getWrongQuestion(familyId, item.id);
}
export async function listWrongQuestions(familyId, input = {}) {
...
export async function getWrongQuestion(familyId, wrongQuestionId) {
...
export async function updateWrongQuestion(familyId, wrongQuestionId, input) {
    await requireWrongQuestion(familyId, wrongQuestionId);
    const data = {
        grade: input.grade,
...
export async function updateWrongQuestionStatus(familyId, wrongQuestionId, input) {
    ...
    if (!WRONG_QUESTION_STATUSES.includes(input.status))
        throw new QuestionBankError("无效的错题状态");
    if (!String(input.reason || "").trim())
        throw new QuestionBankError("人工调整错题状态时必须填写原因");
    await prisma.wrongQuestionEntry.update({
        where: { id: existing.id },
        data: {
            status: input.status,
...
export async function recalculateWrongQuestionMastery(familyId, wrongQuestionId) {
...
export async function deleteWrongQuestion(familyId, wrongQuestionId) {
...
export async function getWrongQuestionPracticeContext(familyId, input) {
    const wrong = await getWrongQuestion(familyId, input.wrong_question_id);
    if (!wrong)
        throw new QuestionBankError("错题不存在", 404);
    const count = Math.min(20, Math.max(1, Number(input.count || 5)));
    const evidence = objectValue(wrong.masteryEvidence);
    const covered = new Set((wrong.attempts || []).map((attempt) => attempt.variationType || attempt.question?.variationType).filter(Boolean));
...
async function validatePaperItems(familyId, childId, questions) {
    if (!Array.isArray(questions) || questions.length === 0)
        throw new QuestionBankError("试卷至少包含一道题");
    if (questions.length > 100)
        throw new QuestionBankError("一份试卷最多包含 100 道题");
    ...
    if (questionRows.length !== questionIds.length)
        throw new QuestionBankError("试卷包含不存在或不属于当前家庭的题目", 404);
    if (wrongRows.length !== wrongIds.length)
        throw new QuestionBankError("试卷包含不存在、学生不一致或不属于当前家庭的错题", 404);
    const wrongMap = new Map(wrongRows.map((item) => [item.id, item]));
    questions.forEach((item, index) => {
        if (!item.question_id)
            throw new QuestionBankError(`试卷第 ${index + 1} 题缺少 question_id`);
...
function paperQuestionData(item, index) {
    return {
...
export async function createPracticePaper(familyId, input) {
    const child = await requireChild(familyId, input.child_id);
    if (!String(input.title || "").trim())
        throw new QuestionBankError("试卷标题不能为空");
    ...
    const paper = await prisma.practicePaper.create({
        data: {
            ...
            totalScore: value(input, "total_score", "totalScore") === undefined
                ? input.questions.reduce((sum, item) => sum + Number(item.score || 0), 0) || undefined
...
export async function listPracticePapers(familyId, input = {}) {
    const { limit, offset } = pageValues(input);
    if (input.child_id)
        await requireChild(familyId, input.child_id);
    const where = {
...
export async function getPracticePaper(familyId, practicePaperId) {
...
export async function updatePracticePaper(familyId, practicePaperId, input) {
    const paper = await requirePracticePaper(familyId, practicePaperId);
    if (input.status && !PAPER_STATUSES.includes(input.status))
        throw new QuestionBankError("无效的试卷状态");
    if (input.questions)
        await validatePaperItems(familyId, paper.childId, input.questions);
    await prisma.$transaction(async (tx) => {
        ...
        if (input.questions) {
            ...
            await tx.practicePaperQuestion.createMany({ data: input.questions.map((item, index) => ({ practicePaperId: paper.id, ...paperQuestionData(item, index) })) });
        }
    });
    return getPracticePaper(familyId, paper.id);
}
export async function deletePracticePaper(familyId, practicePaperId) {
    const paper = await requirePracticePaper(familyId, practicePaperId);
...
async function validateRemediationTasks(familyId, childId, tasks) {
    if (!Array.isArray(tasks) || tasks.length === 0)
        throw new QuestionBankError("教学规划至少包含一个任务");
    if (tasks.length > 100)
        throw new QuestionBankError("一个教学规划最多包含 100 个任务");
    ...
    if (wrongRows.length !== wrongIds.length)
        throw new QuestionBankError("教学任务包含不存在、学生不一致或不属于当前家庭的错题", 404);
    if (typeRows.length !== typeIds.length)
        throw new QuestionBankError("教学任务包含不存在或不属于当前家庭的题型", 404);
    tasks.forEach((task, index) => {
        if (!String(task.title || "").trim() || !String(task.task_type || "").trim())
            throw new QuestionBankError(`第 ${index + 1} 个任务缺少标题或任务类型`);
    });
}
...
export async function saveRemediationPlan(familyId, input) {
    const child = await requireChild(familyId, input.child_id);
    if (!String(input.title || "").trim())
        throw new QuestionBankError("教学规划标题不能为空");
...
export async function listRemediationPlans(familyId, input = {}) {
    const { limit, offset } = pageValues(input);
    if (input.child_id)
        await requireChild(familyId, input.child_id);
    const where = {
...
export async function getRemediationPlan(familyId, remediationPlanId) {
...
export async function updateRemediationPlan(familyId, remediationPlanId, input) {
    const plan = await requireRemediationPlan(familyId, remediationPlanId);
    if (input.status && !PLAN_STATUSES.includes(input.status))
        throw new QuestionBankError("无效的教学规划状态");
    if (input.tasks)
        await validateRemediationTasks(familyId, plan.childId, input.tasks);
    if (input.tasks) {
        const completedTasks = await prisma.remediationTask.count({ where: { planId: plan.id, status: "completed" } });
        if (completedTasks > 0)
            throw new QuestionBankError("教学规划已有完成证据，不能整体替换任务；请只更新规划信息或任务状态", 409);
    }
    await prisma.$transaction(async (tx) => {
        ...
        if (input.tasks) {
            ...
            await tx.remediationTask.createMany({ data: input.tasks.map((task, index) => ({ planId: plan.id, ...taskData(task, index) })) });
        }
    });
    return getRemediationPlan(familyId, plan.id);
}
export async function updateRemediationTaskStatus(familyId, remediationPlanId, taskId, input) {
    await requireRemediationPlan(familyId, remediationPlanId);
    if (!TASK_STATUSES.includes(input.status))
        throw new QuestionBankError("无效的任务状态");
    const task = await prisma.remediationTask.findFirst({ where: { id: taskId, planId: remediationPlanId, plan: { familyId } } });
    if (!task)
        throw new QuestionBankError("教学任务不存在或不属于当前家庭", 404);
...
export async function deleteRemediationPlan(familyId, remediationPlanId) {
    const plan = await requireRemediationPlan(familyId, remediationPlanId);
...
export async function recordQuestionAttemptWithWrongBook(familyId, input) {
