...
vi.mock("../prisma.js", () => ({
    prisma: {
        tutorMessage: { findMany: (...args) => findManyMessage(...args) },
        tutorConversation: {
            findUnique: (...args) => findUniqueConversation(...args),
            update: (...args) => updateConversation(...args),
        },
        evidenceRecord: { findFirst: (...args) => findFirstEvidence(...args) },
    },
}));
vi.mock("../v2/evidence.js", () => ({
    createEvidenceRecord: (...args) => createEvidence(...args),
}));
...
describe("长期记忆：证据回流", () => {
    it("写入时为待确认状态，来源标为 tutor", async () => {
        ...
        expect(createEvidence).toHaveBeenCalledWith("family-1", expect.objectContaining({ childId: "child-1", source: "tutor", sourceRef: "conv-1" }), { type: "tutor", id: "user-1" });
    });
