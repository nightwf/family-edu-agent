...
const prismaMock = new Proxy({
    ...
    tutorConversation: {
        findFirst: vi.fn(async ({ where }) => where.id === state.conversationId && where.familyId === "family-1"
...
const savedFiles = [];
vi.mock("../storage.js", () => ({
    saveFile: vi.fn(async (key, buffer, contentType) => {
...
function multipart(filename, contentType, bytes, field = "file") {
