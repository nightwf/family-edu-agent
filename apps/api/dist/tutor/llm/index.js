...
import { FakeChatProvider } from "./types.js";
...
/** 仅供测试：注入供应商并绕过环境变量。 */
export function setChatProvider(provider) {
    cached = provider;
}
...
/** 有图片时优先用视觉模型，否则用对话模型。 */
export function pickModel(hasImages) {
    if (hasImages && env.TUTOR_VISION_MODEL)
        return env.TUTOR_VISION_MODEL;
