...
const writes = { evidence: [], message: [] };
const evidenceCreate = vi.fn(async ({ data }) => {
...
const prismaMock = {
    ...
    tutorConversation: {
        ...
        findMany: vi.fn(async ({ where }) => {
            ...
            return Object.values(CONVERSATIONS).filter((conversation) => conversation.familyId === where.familyId &&
                (where.childId === undefined || conversation.childId === where.childId) &&
                (where.status === undefined || conversation.status === where.status));
        }),
        create: vi.fn(async ({ data }) => ({ id: "conv-new", ...data })),
        update: vi.fn(async () => ({})),
    },
    tutorMessage: {
        count: vi.fn(async () => 0),
    ...
    auditLog: { create: vi.fn(async ({ data }) => ({ id: "audit-1", ...data })) },
};
// 未显式打桩的模型一律返回空值
prismaMock.$proxy = new Proxy(prismaMock, {
    get: (target, prop) => prop in target
        ? target[prop]
        : new Proxy({}, { get: (_t, method) => vi.fn().mockResolvedValue(method === "count" ? 0 : method === "findFirst" || method === "findUnique" ? null : []) }),
});
...
describe("同家庭不同孩子：会话与消息隔离", () => {
    it("按孩子筛会话时，查询条件必须同时带上 childId 与 familyId", async () => {
        ...
        expect(conversations.map((row) => row.id)).toEqual(["conv-2"]);
    });
    ...
    it("读老二的会话，取到的是老二的消息与他自己的身份", async () => {
        ...
        expect(body.messages.every((row) => row.childId === "child-2")).toBe(true);
