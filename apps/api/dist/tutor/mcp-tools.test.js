...
vi.mock("../prisma.js", () => ({
    prisma: new Proxy({
        child: {
            findFirst: (...args) => childFindFirst(...args),
            findMany: (...args) => childFindMany(...args),
        },
    }, { get: (target, prop) => (prop in target ? target[prop] : anyModel()) }),
}));
...
async function open(persona, pinnedChildId) {
...
describe("私教工具集（与真实 MCP 服务器对接）", () => {
    ...
    it("结果过长会截断，避免整本错题本塞进上下文", async () => {
        childFindMany.mockResolvedValue(Array.from({ length: 400 }, (_, index) => ({ id: `child-${index}`, name: `孩子${index}` })));
        const toolset = await open("child_tutor", "child-1");
