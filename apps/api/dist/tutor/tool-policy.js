...
export function allowedToolNames(persona) {
 ...
 * 避免给不接受该参数的工具塞多余字段。
 */
export function sanitizeToolArguments(toolName, rawArguments, context, acceptsChildId = true) {
 ...
 * 返回 null 表示不过滤。
 */
export function filterChildrenResult(persona, pinnedChildId, payload) {
    if (normalizePersona(persona) !== "child_tutor" || !pinnedChildId)
        return payload;
    const list = Array.isArray(payload) ? payload : Array.isArray(payload?.children) ? payload.children : null;
    if (!list)
        return payload;
    const filtered = list.filter((item) => item?.child_id === pinnedChildId || item?.id === pinnedChildId);
