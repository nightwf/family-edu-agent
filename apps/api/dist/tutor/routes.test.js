...
const prismaMock = new Proxy({
    ...
    child: {
        ...
        findFirst: vi.fn(async ({ where }) => (where.id === state.ownedChildId && where.familyId === "family-1" ? { id: where.id, familyId: where.familyId, name: "JOJO" } : null)),
    },
    tutorConversation: {
        create: vi.fn(async ({ data }) => ({ id: "conv-new", ...data })),
        // conv-1 属于 family-1；换家庭身份后必须查不到。
        findFirst: vi.fn(async ({ where }) => (where.id === state.conversationId && where.familyId === "family-1" ? { id: where.id, familyId: where.familyId, childId: state.ownedChildId, persona: "child_tutor", status: "active", title: null } : null)),
