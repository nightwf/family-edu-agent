...
vi.mock("./safety.js", () => ({
    scanInput: (text) => {
        ...
        if (String(text).includes("自杀"))
            return { ok: false, reason: "自伤相关表达", action: "blocked" };
        return { ok: true, action: "pass" };
    },
    scanOutput: (text) => {
        if (String(text).includes("你真笨"))
            return { ok: false, reason: "羞辱或比较式表达", action: "replaced", text: "我们换个说法" };
        return { ok: true, action: "pass", text };
    },
    validateReferences: (...args) => validateReferences(...args),
    recordSafetyEvent: (...args) => recordSafetyEvent(...args),
    moderationConfigured: () => true,
}));
/** 按轮次吐事件的假供应商。 */
function providerFromRounds(rounds) {
    let index = 0;
    return {
        name: "test",
        supportsVision: true,
...
async function collect(input) {
    const events = [];
    for await (const event of runTutorTurn(input))
        events.push(event);
    return events;
}
beforeEach(() => {
...
describe("Agent 循环", () => {
    ...
    it("工具轮次超限会停止调用并要求收尾", async () => {
        ...
        // 2 轮上限 + 收尾轮 = 最多 3 次调用
        expect(toolset.callTool.mock.calls.length).toBeLessThanOrEqual(3);
