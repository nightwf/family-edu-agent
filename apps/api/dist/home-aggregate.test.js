...
describe("home aggregate", () => {
    ...
    it("学科概览算不出来时不影响首页其它数据", async () => {
        ...
        const result = await loadHomeAggregate("family-1", "child-1");
        expect(result.subject_overview).toBeNull();
        expect(result.planning_request).toMatchObject({ id: "plan-1" });
        expect(result.child_state).toMatchObject({ summary: { evidence_7d: 4 } });
    });
    it("待规划事项算不出来时学科概览照常返回", async () => {
        mocks.priorities.mockRejectedValue(new Error("learning engine unavailable"));
        const result = await loadHomeAggregate("family-1", "child-1");
        expect(result.learning_priorities).toBeNull();
        expect(result.planning_request).toBeNull();
        expect(result.subject_overview).toMatchObject({ overall: { conclusion: "数学需要优先处理" } });
    });
});
