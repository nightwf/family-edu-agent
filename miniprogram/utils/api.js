...
const api = {
  ...
  reviewFamilyJoinRequest: (id, data) => request({ url: `/api/onboarding/family/join-requests/${id}/review`, method: "POST", data }),
  connections: () => request({ url: "/api/connections" }),
  revokeConnection: (id) => request({ url: `/api/connections/${id}`, method: "DELETE" }),
  workbuddyBindSession: (publicId, data) => request({ url: `/api/oauth/bindings/${publicId}/wechat-session`, method: "POST", data, auth: false }),
  workbuddyApprove: (publicId, data) => request({ url: `/api/oauth/bindings/${publicId}/approve`, method: "POST", data }),
  webLoginSession: (publicId, data) => request({ url: `/api/auth/wechat/web/${publicId}/wechat-session`, method: "POST", data, auth: false }),
  webLoginApprove: (publicId, data) => request({ url: `/api/auth/wechat/web/${publicId}/approve`, method: "POST", data }),
  ...
  childState: (childId) => request({ url: `/api/v2/children/${childId}/state` }),
  childEducationProfile: (childId) => request({ url: `/api/children/${childId}/education-profile` }),
  updateChildEducationProfile: (childId, data) => request({ url: `/api/children/${childId}/education-profile`, method: "PATCH", data }),
  childEvidence: (childId, params) => request({ url: `/api/v2/children/${childId}/evidence?${qs(params)}` }),
  childRelationship: (childId) => request({ url: `/api/v2/children/${childId}/relationship` }),
  relationshipHistory: (childId, params) => request({ url: `/api/v2/children/${childId}/relationship/history?${qs(params)}` }),
  reviewEvidence: (id, data) => request({ url: `/api/v2/evidence/${id}/review`, method: "PATCH", data }),
  listStageGoals: (childId, params) => request({ url: `/api/v2/children/${childId}/goals?${qs(params)}` }),
  getStageGoal: (id) => request({ url: `/api/v2/goals/${id}` }),
