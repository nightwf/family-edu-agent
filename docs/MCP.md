...
| 工具 | 关键参数 | 用途 |
| --- | --- | --- |
| `get_subject_overview` | `child_id` | 读取该孩子各学科的整体概览，包含没有数据的学科 |

回答"哪一科要优先处理""各科现在什么情况"这类问题时先读这个工具，再配合 `get_learning_priorities` 说明依据；
不要拿单点成绩或单条记录推断学科全貌。返回结果会带上进度、掌握度、待处理错题和近期动态，
没有数据的学科也会列出并给出空状态说明，不是过滤掉。
