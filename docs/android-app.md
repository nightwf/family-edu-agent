...
- **内置学习私教入口**：App 的 WebView 在 UA 里附加 `HeYaAndroid/1.0`，网页据此显示"学习私教"页，
  桌面浏览器和微信小程序不显示这个入口。因为入口是网页根据 UA 判定的，私教改动**不需要重新发版**；
  只有麦克风权限这类原生能力才需要出新 APK。
- **麦克风权限**：私教支持按住说话，App 声明了 `RECORD_AUDIO` 与 `MODIFY_AUDIO_SETTINGS`，
  并在 WebView 请求录音权限时转成系统授权弹窗。用户拒绝后会提示改用文字输入，不阻塞其他功能。
