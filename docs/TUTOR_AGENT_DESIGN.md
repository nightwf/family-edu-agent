...
### 2.1 与「WorkBuddy Agent Gateway」原则的关系

`docs/ARCHITECTURE_PROPOSAL_V2.md` 3.3 定过一条原则：模型能力由 WorkBuddy 提供，禾芽不重复建设第二套大模型能力。本方案的私教运行时由禾芽直接调用模型，看起来与之相抵，需要讲清为什么不冲突。

那条原则针对的是**生成类**任务（生成候选目标、周计划、题目、长文）。本方案不改变这部分：需要规划时，私教不自己规划，而是走现有的 `PlanningRequest` 流程交给 WorkBuddy，闭环仍然只有一条。私教承载的是**实时对话运行时**，两者的差别是结构性的：
