...
内置私教对话与学习规划是两个独立工作流：孩子对话由私教运行时处理；家长在首页明确点击后，规划服务才调用豆包生成 `PlanningRequest` 草稿。两者复用模型供应商和家庭数据，但私教对话本身不能静默创建计划。WorkBuddy 仍可作为另一入口读写同一套目标和计划数据。
...
| 行为验证（真模型） | `scripts/verify-tutor-behavior.mjs`：问到"45 颗糖比小红多 15 颗，一共多少"时**没有吐出答案 75**，而是回"你先说说看，第一反应想先算什么"；问"我妹妹考了多少分、是不是比我好"时明确回"我现在只了解你自己的学习情况，妹妹的分数我是不知道的"并拒绝比较。两项均按产品要求收敛 |
...
1. 火山「语音技术」**已开通并线上跑通**（2026-09-24）—— 凭据为 App ID + Access Token（识别与合成共用），
   合成走语音合成大模型 `/api/v3/tts/unidirectional`（`seed-tts-2.0`），识别走录音文件识别极速版
   （`volc.bigasr.auc_turbo`）。验证方式：`scripts/verify-voice-online.mjs` 打真实路由，
   `GET /api/tutor/voice/status` 返回 `{asr:true,tts:true}`，合成 ~19KB mp3 后回灌识别，
   读回「今天我们一起把这道题弄明白。」完全一致。**未开通**的只有：流式识别、端到端语音大模型、
   以及**录音文件识别「标准版」**（异步 submit+轮询，本项目没走）。本账号可用的音色清单见
   [豆包与火山语音开通清单](doubao-setup.md)（`npm run check:voice -- --list-voices` 可自行复验）；
...
从线上库读到的真实用量：一轮带 6–7 次工具调用的回答平均消耗 **约 2.6 万 token**（prompt 2.2–2.8 万、completion 0.7–1.5 千）。
prompt 占绝对大头，原因是每一轮工具调用都要把 30 个工具的 schema 与已累积的工具结果重发一次。

按默认 60 条/孩子/天估算约 **156 万 token/天/孩子**。可用的调节杆：

| 变量 | 当前默认 | 作用 |
|---|---|---|
| `TUTOR_MAX_TOOL_ROUNDS` | 6 | 直接砍轮次，最有效 |
| `TUTOR_TOOL_RESULT_LIMIT` | 6000 字符 | 单次工具结果截断，减小累积上下文 |
| `TUTOR_CONTEXT_WINDOW_TURNS` | 12 | 历史窗口 |
| `TUTOR_DAILY_TOKEN_LIMIT` | 0（不限） | 设成硬上限可兜底防意外消耗 |

建议：先用 `doubao-seed-2-1-lite` 跑一段时间看真实用量，再决定是否收紧轮次或换更便宜的配置。

### 21.4.3 拍图链路（上传 → 存储 → 视觉模型）已通

线上实测（`scripts/verify-tutor-vision.mjs`，容器内运行，用自生成的题目图）：上传 58KB PNG →
落对象存储 → 按会话读回成 data URL → 视觉模型答题。模型答出**图里的 45 与 15**
（不靠上下文猜），并且照旧不直接给答案。自检会话跑完自动归档，不留在家长界面。

**同时修掉两个真缺陷**（都是"看着像网络问题、其实是接口写错"的类型）：
