...
- 测试基线：API 25 文件 123 用例通过 + 小程序 18 页校验通过
...
4. **WorkBuddy 对接完全不变。** 连接协议、`/family-edu/mcp` 路径、已提交开放平台的 Connector/Skill/Expert 包都不动。
5. **不自建模型能力。** 不训练自有大模型，不自研 ASR/TTS，不做声纹克隆。
6. **不新写第二套工具集。** 复用现有 MCP 工具（`apps/api/src/mcp.ts` 81 个 + `apps/api/src/v2/mcp-tools.ts` 42 个），以进程内 MCP 客户端调用。
...
- MCP 工具只增不改：不得修改任何现有工具的名称、参数与返回结构；`get_sync_spec` 只做增量补充并把版本号递进（当前 `2.5` → `2.6`），不改已有字段语义与 `enums` 取值。
...
17. 现有 API 全部测试通过（不低于 25 文件 123 用例）。
18. 小程序 18 页校验通过。
...
1. **对话与视觉模型供应商 + API Key**（阻塞第 2 步）
2. **ASR/TTS 供应商 + 账号凭据**（阻塞第 9 步）
3. **内容审核服务商 + 凭据**（阻塞第 5 步）

待确认项（不阻塞开工，可边做边定）：

1. 每日配额默认值（当前建议每孩子 60 条消息，家长可配）；
2. 儿童语音原始音频是否留存（当前建议不留存）；
3. 是否随本批一起上线"教育方式按孩子维度"（暂存区代码，影响第 4 步人格口径）。
